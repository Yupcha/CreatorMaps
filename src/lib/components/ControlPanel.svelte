<script lang="ts">
  ...
  function applyPreset(preset: MapPreset) {
    ...
    viewSettings.update((v) => ({
      ...v,
      terrainExaggeration: preset.exaggeration,
    }));
    ...
    <div class="panel-header">
      <div class="panel-brand">
        <div class="brand-left">
          ...
          <h1 class="brand-title">Yupcha Maps</h1>
        </div>
        <button
          class="btn btn-icon"
          onclick={togglePanel}
          title="Collapse panel"><X size={16} /></button
        >
      </div>
    ...
    <div class="panel-content">
        ...
        <div class="tab-section">
          ...
          <div class="slider-container">
            ...
            <input
              ...
              oninput={(e) =>
                visualFilters.update((f) => ({
                  ...f,
                  brightness: Number(e.currentTarget.value),
                }))}
            />
            <span class="slider-value"
              >{$visualFilters.brightness.toFixed(2)}</span
            >
          </div>
          <div class="slider-container">
            ...
            />
            <span class="slider-value"
              >{$visualFilters.contrast.toFixed(2)}</span
            >
          </div>
          <div class="slider-container">
            ...
            <input
              ...
              max="2"
              step="0.05"
              value={$visualFilters.saturation}
              oninput={(e) =>
                visualFilters.update((f) => ({
                  ...f,
                  saturation: Number(e.currentTarget.value),
                }))}
            />
            <span class="slider-value"
              >{$visualFilters.saturation.toFixed(2)}</span
            >
          </div>
          <div class="slider-container">
            ...
            <input
              ...
              max="360"
              step="5"
              value={$visualFilters.hueRotate}
              oninput={(e) =>
                visualFilters.update((f) => ({
                  ...f,
                  hueRotate: Number(e.currentTarget.value),
                }))}
            />
          ...
          <div class="slider-container">
            ...
            <input
              ...
              max="100"
              step="5"
              value={$visualFilters.sepia}
              oninput={(e) =>
                visualFilters.update((f) => ({
                  ...f,
                  sepia: Number(e.currentTarget.value),
                }))}
            />
          ...
          <div class="slider-container">
            ...
            <input
              ...
              oninput={(e) =>
                visualFilters.update((f) => ({
                  ...f,
                  grayscale: Number(e.currentTarget.value),
                }))}
            />
          ...
          <div class="chip-grid" style="grid-template-columns: repeat(2, 1fr);">
            {#each ["png", "jpeg"] as const as fmt}
              <button
                ...
                onclick={() =>
                  exportSettings.update((s) => ({ ...s, format: fmt }))}
              >
                {fmt.toUpperCase()}
              </button>
            {/each}
          ...
          {#if $exportSettings.format === "jpeg"}
            ...
            <div class="slider-container">
              ...
              <input
                ...
                oninput={(e) =>
                  exportSettings.update((s) => ({
                    ...s,
                    quality: Number(e.currentTarget.value),
                  }))}
              />
              <span class="slider-value"
                >{Math.round($exportSettings.quality * 100)}%</span
              >
            </div>
          {/if}
          ...
          <div class="slider-container">
            ...
            <input
              ...
              oninput={(e) =>
                exportSettings.update((s) => ({
                  ...s,
                  pixelRatio: Number(e.currentTarget.value),
                }))}
            />
      ...
      {#if $activeTab === "presets"}
        <div class="tab-section">
          ...
          <p
            style="font-size: 10px; color: var(--text-tertiary); margin-bottom: 8px;"
          >
            Highlight cities by industry & tag
          </p>
          <div
            class="chip-grid"
          ...
          >
            <button
              class="chip"
              class:active={$activeThematicFilter === null}
              onclick={() => activeThematicFilter.set(null)}>All Cities</button
            >
            <button
              class="chip"
              class:active={$activeThematicFilter === "tech"}
              onclick={() => activeThematicFilter.set("tech")}
              >💻 Tech Hubs</button
            >
            <button
              class="chip"
              class:active={$activeThematicFilter === "film"}
              onclick={() => activeThematicFilter.set("film")}
              >🎬 Film & Cinema</button
            >
            <button
              class="chip"
              class:active={$activeThematicFilter === "tourism"}
              onclick={() => activeThematicFilter.set("tourism")}
              >🏰 Tourism</button
            >
            <button
              class="chip"
              class:active={$activeThematicFilter === "textiles"}
              onclick={() => activeThematicFilter.set("textiles")}
              >🧵 Textiles</button
            >
            <button
              class="chip"
              class:active={$activeThematicFilter === "port"}
              onclick={() => activeThematicFilter.set("port")}
              >⚓ Major Ports</button
            >
          </div>
        ...
        <div class="tab-section">
          ...
          <div class="control-row">
            ...
            <label class="toggle">
              <input
                type="checkbox"
                checked={$stateOverlayVisible}
                onchange={(e) =>
                  stateOverlayVisible.set(e.currentTarget.checked)}
              />
          ...
          {#if $stateOverlayVisible}
            ...
            >
              <div class="segmented-control">
                <button
                  class="segment"
                  class:active={$boundaryDetail === "states"}
                  onclick={() => boundaryDetail.set("states")}>States</button
                >
                <button
                  class="segment"
                  class:active={$boundaryDetail === "districts"}
                  onclick={() => boundaryDetail.set("districts")}
                  >Districts</button
                >
              </div>
            </div>
          ...
          <div class="chip-grid">
            {#each metricOptions as opt}
              <button
                class="chip"
                class:active={$overlayMetric === opt.key}
          ...
          <div class="chip-grid" style="margin-top: 4px;">
            {#each [0.25, 0.5, 0.75, 1] as preset}
              <button
                class="chip chip-sm"
                class:active={Math.abs($overlayOpacity - preset) < 0.03}
          ...
          <div class="legend-bar">
            <div
              ...
              style:background="linear-gradient(90deg, {currentMetricConfig
                .colors[0]}, {currentMetricConfig.colors[1]}, {currentMetricConfig
                .colors[2]})"
            ></div>
          ...
          {#if $cityPinsVisible}
            ...
            <div class="pin-legend">
              <div class="pin-legend-item">
                <span class="pin-dot" style="background:#6366f1"></span> Metro (10M+)
              </div>
              <div class="pin-legend-item">
                <span class="pin-dot" style="background:#06b6d4"></span> Tier 1 (3-10M)
              </div>
              <div class="pin-legend-item">
                <span class="pin-dot" style="background:#f59e0b"></span> Tier 2
              </div>
              <div class="pin-legend-item">
                <span class="pin-dot" style="background:#10b981"></span> Tier 3 /
                Hill
              </div>
              <div class="pin-legend-item">
                <span class="pin-dot" style="background:#a855f7"></span> UT Capital
              </div>
              <div class="pin-legend-item">
                <span class="pin-dot" style="background:#ef4444"></span> Landmark
              </div>
            </div>
          {/if}
          ...
          <div class="india-ranks">
            {#each topStates as state, i}
              ...
              >
                ...
                <span class="rank-state-val"
                  >{currentMetricConfig.format(
                    state[$overlayMetric] as number,
                  )}</span
                >
              </button>
            {/each}
          ...
          <button
            class="btn"
            style="margin-top: 12px; width: 100%; display: flex; align-items: center; justify-content: center; gap: 6px;"
            onclick={flyToIndia}
          >
          ...
          <div class="control-row">
            <span class="control-label"
              ><Box size={14} style="display:inline" /> 3D Data Bars</span
            >
            <label class="toggle">
              <input
                type="checkbox"
